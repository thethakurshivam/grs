...
const createPOCDummyData = async () => {
  try {
    console.log('Creating POC dummy data in sitaics database...');
    ...
    // Step 4: Create POC user with the MOU IDs
    const pocUser = new poc({
      ...
      mous: createdMOUs.map((mou) => mou._id),
    });

    await pocUser.save();
    console.log(`Created POC user: ${pocUser.name} (Email: ${pocUser.email})`);
    ...
    console.log(
      '\n✅ POC dummy data created successfully in sitaics database!'
    );
    console.log(`📊 Summary:`);
    ...
    console.log(`   - Created POC user with MOU associations`);
    console.log('\n🎯 Demo Credentials:');
