...
async function testPOCClaims() {
  try {
    console.log('🔍 Testing POC BPRND Claims Endpoint...\n');

    // Test the POC claims endpoint
    ...
    const response = await fetch('http://localhost:3003/api/bprnd/claims', {
      ...
      headers: { 
        ...
        'Authorization': 'Bearer poc-token-placeholder' // This will fail auth but we can see the response structure
      }
    });
    ...
    console.error('❌ Error testing POC claims:', error);
  }
}
