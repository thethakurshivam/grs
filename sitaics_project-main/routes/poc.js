...
// Health check for POC routes
router.get('/health', (req, res) => {
  res.json({ 
    success: true, 
    service: 'POC API',
    message: 'POC routes are healthy',
...
router.get('/:pocId/courses', authenticatePOC, async (req, res) => {
  try {
    const { pocId } = req.params;

    // Verify that the authenticated POC is accessing their own data
    if (req.user.pocId !== pocId && req.user.userId !== pocId) {
      return res.status(403).json({
        success: false,
        error: 'Access denied. You can only access their own data.'
      });
    }
...
router.get('/:pocId/mous', authenticatePOC, async (req, res) => {
  try {
    const { pocId } = req.params;
...
router.get('/:pocId/students', authenticatePOC, async (req, res) => {
  try {
    const { pocId } = req.params;
...
router.post('/:pocId/bulk-import-students', authenticatePOC, upload.single('file'), async (req, res) => {
  try {
    const { pocId } = req.params;
...
router.get('/test-email-config', authenticatePOC, async (req, res) => {
  try {
