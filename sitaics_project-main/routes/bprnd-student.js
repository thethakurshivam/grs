...
// Import models - EXACT same as api4.js
...
// JWT Secret
const JWT_SECRET = process.env.JWT_SECRET || 'bprnd-student-secret-key-change-in-production';

// Configure multer for file uploads - EXACT same as api4.js
...
router.get('/health', (req, res) => {
  res.json({ 
    success: true, 
    service: 'BPR&D Student API',
    message: 'BPR&D Student routes are healthy',
    timestamp: new Date().toISOString()
  });
});

// EXACT COPY OF ALL ROUTER ROUTES FROM api4.js - START
...
// UMBRELLAS ROUTE - Must come before /:id
...
router.get('/:id/credits/breakdown', async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/:id/course-history/:umbrella', async (req, res) => {
  try {
...
router.post('/api/bprnd/student/login', async (req, res) => {
  try {
    const { email, password } = req.body;
...
router.post('/:id/claim-certification', async (req, res) => {
  try {
    const { id } = req.params;
...
router.post('/:id/umbrella/:umbrella/redeem/:qualification', async (req, res) => {
  try {
...
router.post('/:id/certifications/request', async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/certificate/:id/pdf', async (req, res) => {
  try {
    ...
    try {
      const verificationUrl = `http://12.10.11.83:3004/verify/certificate/${cert._id}`;
      ...
      // Add QR Code to bottom right of certificate
      doc.moveDown(1);
      doc.image(qrCodeDataURL, doc.page.width - 200, doc.page.height - 200, { width: 120, height: 120 });
      
      // Add QR Code label
      doc.fontSize(8).text('Scan QR Code to Verify', doc.page.width - 200, doc.page.height - 80, { width: 120, align: 'center' });
      doc.fontSize(6).text('Certificate Authenticity', doc.page.width - 200, doc.page.height - 70, { width: 120, align: 'center' });
      
      console.log(`✅ QR Code generated for certificate ${cert._id}`);
...
// EXACT COPY OF ALL ROUTER ROUTES FROM api4.js - END

// Certificate verification endpoint for QR code scanning
router.get('/verify/certificate/:id', async (req, res) => {
  try {
    ...
    // Fetch certificate with student details
    const certificate = await BprndCertificate.findById(id)
      .populate('studentId', 'Name email EmployeeId Designation State Department')
      .lean();
    ...
    res.status(200).json({
      ...
      certificate: {
        ...
        umbrella: certificate.umbrellaKey.replace(/_/g, ' '),
        issuedAt: certificate.issuedAt,
        student: {
          name: certificate.studentId?.Name || 'N/A',
          employeeId: certificate.studentId?.EmployeeId || 'N/A',
          designation: certificate.studentId?.Designation || 'N/A',
          state: certificate.studentId?.State || 'N/A',
          department: certificate.studentId?.Department || 'N/A'
        },
