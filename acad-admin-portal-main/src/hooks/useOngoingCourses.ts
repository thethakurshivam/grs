...
export const useOngoingCourses = () => {
  ...
  const fetchOngoingCourses = async () => {
    try {
      ...
      console.log('Making API request to:', 'http://localhost:3000/admin/courses/ongoing');
      
      const response = await fetch('http://localhost:3000/admin/courses/ongoing', {
        method: 'GET',
        headers: {
      ...
      if (data.success) {
        setOngoingCourses(data.data);
        setOngoingCoursesCount(data.count);
